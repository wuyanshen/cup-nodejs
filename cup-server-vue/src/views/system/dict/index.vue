  ...
  <div class="app-container">
    ...
    <el-form v-show="showSearch" ref="queryForm" :model="queryForm" inline size="small">
      <el-form-item label="字典名称" label-width="80px" prop="name">
        <el-input v-model="queryForm.name" placeholder="请输入字典名称" type="text" clearable style="width: 230px" />
      </el-form-item>
      <el-form-item label="字典类型" label-width="80px" prop="type">
        <el-input v-model="queryForm.type" placeholder="请输入字典类型" type="text" clearable style="width: 230px" />
      </el-form-item>
      ...
      <el-table-column label="操作" min-width="230" align="center">
        <template slot-scope="scope">
          ...
          <el-button
            ...
            icon="el-icon-s-grid"
            size="mini"
            @click="handleToItem(scope.row.id)"
          >字典项</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :visible.sync="open" width="600px" :close-on-click-modal="false" append-to-body :title="title">
      <el-form ref="form" :model="form" :rules="rules" label-width="80px" size="small">
        <el-form-item label="字典名称" prop="name">
          <el-input v-model="form.name" placeholder="请输入字典名称" type="text" clearable />
        </el-form-item>
        <el-form-item label="字典类型" prop="type">
          <el-input v-model="form.type" placeholder="请输入字典类型" type="text" clearable />
        </el-form-item>
...
<script>
import { add, checkDictType, del, retrieve, update } from '@/api/dict/dict'
...
export default {
  ...
  methods: {
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          if (this.form.id === undefined) {
            checkDictType({ type: this.form.type }).then(res => {
              if (res === 1) {
                return this.msgError('字典类型已存在')
              } else {
    ...
    // 删除按钮
    handleDelete(row) {
      this.$confirm(`您是否要删除字典"${row.name}"?`, '警告', {
    ...
    // 字典项按钮
    handleToItem(dictId) {
      this.$router.push(`/dict/item/index/${dictId}`)
    },
    // 重置按钮
    resetQuery(refName) {
